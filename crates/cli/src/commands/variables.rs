...
use crate::consts::{NO_SERVICE_LINKED, SERVICE_NOT_FOUND};
...
struct Plugin<'a>(&'a queries::project::ProjectProjectPluginsEdgesNode);
...
impl<'a> Display for Plugin<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            "{}",
            match &self.0.name {
                queries::project::PluginType::mongodb => "MongoDB",
                queries::project::PluginType::mysql => "MySQL",
                queries::project::PluginType::postgresql => "PostgreSQL",
                queries::project::PluginType::redis => "Redis",
                queries::project::PluginType::Other(plugin) => plugin,
            }
        )
    }
}
